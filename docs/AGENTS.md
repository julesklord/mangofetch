...
- **Rust 1.70+** is required (repo now pins toolchain via rust-toolchain.toml).
- **GUI:** the desktop GUI uses `egui`/`eframe` (no Node/Tauri build step in this repository). If you are building any Tauri-based UI from forks, follow that project's Node/pnpm instructions.
- **Dependencies:** `yt-dlp` and `FFmpeg` are auto-downloaded to the app data dir on first run. Use `mangofetch-cli check` to verify.

Note: CI and tests run with `MANGOFETCH_OFFLINE=1` to avoid network downloads — use that env var for reproducible test runs.
